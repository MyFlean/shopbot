...
def _product_matches_filters(src: Dict[str, Any], filters: Dict[str, Any]) -> bool:
    """
    Check if a raw ES _source doc matches the given filters (in-memory).
    Same logic as _build_filter_clauses but applied to a single document.
    """
    if not filters or not isinstance(filters, dict):
        return True
    ...
    if price_range and price_range in PRICE_RANGE_FILTERS:
        ...
        try:
            ...
            return False
        pr = PRICE_RANGE_FILTERS[price_range]
        if "lt" in pr and p >= pr["lt"]:
            return False
        if "lte" in pr and p > pr["lte"]:
            return False
        if "gte" in pr and p < pr["gte"]:
            return False
        if "gt" in pr and p <= pr["gt"]:
            return False

    # 2. Flean score (subcategory_percentile)
    flean_score = filters.get("flean_score")
    if flean_score and flean_score in FLEAN_SCORE_FILTERS:
        percentile = None
        try:
            percentile = (
                (src.get("stats") or {})
                .get("adjusted_score_percentiles") or {}
            ).get("subcategory_percentile")
        ...
        try:
            ...
            return False
        fs = FLEAN_SCORE_FILTERS[flean_score]
        if "gte" in fs and pct < fs["gte"]:
            return False
        if "lte" in fs and pct > fs["lte"]:
            return False

    # 3. Preferences (OR - at least one term must match)
    preferences = filters.get("preferences", [])
    if isinstance(preferences, list) and preferences:
        preference_terms: List[str] = []
        for pref in preferences:
        ...
        if preference_terms:
            searchable = " ".join(
                _as_list(src.get("name"))
                + _as_list(src.get("description"))
                + _as_list((src.get("package_claims") or {}).get("dietary_labels"))
                + _as_list((src.get("package_claims") or {}).get("health_claims"))
            ).upper()
            if not any(term.upper() in searchable for term in preference_terms):
                return False

    # 4. Dietary (AND - each restriction must match)
    dietary = filters.get("dietary", [])
    if isinstance(dietary, list) and dietary:
        for diet_filter in dietary:
            if diet_filter in DIETARY_FILTERS:
                ...
                if not any(term.upper() in searchable for term in diet_terms):
                    return False
...
def _as_list(x: Any) -> List[str]:
    """Convert value to list of strings for searchable text."""
    if x is None:
        return []
    if isinstance(x, str):
...
def filter_products_in_memory(
    products: List[Dict[str, Any]], filters: Optional[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    Filter a list of raw ES _source docs by personalization filters.
    Used by curated API when filters are provided.
    """
    if not filters or not isinstance(filters, dict):
        return products
    return [p for p in products if _product_matches_filters(p, filters)]
...
class ElasticsearchProductsFetcher:
    ...
    def search_recommended_products(
        ...
        """Find recommended products for PDP (similar products in same subcategory).

        Logic:
        1. Fetch the source product to get its category_paths.
        2. Search the same subcategory, sorted by subcategory_percentile DESC.
        3. Exclude the source product.
        4. Return the top ``limit`` raw _source dicts.

        Returns dict with:
          - 'products': list of raw _source dicts (ready for transform_to_product_card)
          - 'source_product_id': the original product ID
          - 'subcategory': the subcategory used for matching
          - 'total_in_subcategory': total products in that subcategory
        """
        result: Dict[str, Any] = {
            "products": [],
        ...
        cat_paths = src.get("category_paths", [])
        if not cat_paths:
            print(f"DEBUG: search_recommended_products | no category_paths for {product_id}")
            return result
        ...
        try:
            ...
            if isinstance(total_hits, dict):
                result["total_in_subcategory"] = total_hits.get("value", 0)
            else:
                result["total_in_subcategory"] = total_hits or 0
            
            hits = hits_data.get("hits", []) or []
            products = [h.get("_source", {}) for h in hits if h.get("_source")][:limit]
            print(f"DEBUG: ES recommended_products | found={len(products)} | total={result['total_in_subcategory']}")
            result["products"] = products
            return result

        except Exception as exc:
            print(f"DEBUG: ES recommended_products failed | error={exc}")
            return result
    ...
    def search_products_unified(
        ...
        """Unified product search with text query, subcategory filtering, pagination, and filters.
        
        This method combines the capabilities of:
        - search(): Text query with filters (no pagination)
        - search_by_subcategory(): Category browsing with pagination (no filters)
        
        Args:
            query: Optional text search query (e.g., "protein bars")
            subcategory: Optional ES category path (e.g., "f_and_b/food/light_bites/chips_and_crisps")
            page: Page number, 0-indexed (default 0)
            size: Items per page, 1-100 (default 20)
            sort_by: Sort option - relevance, price_asc, price_desc, protein_desc, fiber_desc, fat_asc
            filters: Optional filter object with price_range, flean_score, preferences, dietary
        ...
        try:
            ...
            print(f"DEBUG: ES search_products_unified timeout | query={query} | subcategory={subcategory}")
            return {
                "products": [],
                "meta": {
                    "total": 0,
            ...
            print(f"DEBUG: ES search_products_unified failed | query={query} | subcategory={subcategory} | error={exc}")
            return {
                "products": [],
                "meta": {
                    "total": 0,
