...
def get_product_details_batch() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # A missing individual id within a successful batch already surfaces
        # via `not_found` (no per-item fallback needed).
...
def _build_filters_from_query_args() -> Dict[str, Any]:
    """Build a filters dict from GET query params (shared by search and home endpoints)."""
...
def _normalize_filter_aliases(filters: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    _validate_filters can handle the result.
...
def get_products_unified() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        # Execute unified search — same V2 routing as unified_search.py's
        # /rs/v1/search (this endpoint is a documented functional subset of
        # it: no in_stock/cta/lab_report enrichment).
        ...
        # V2-only for subcategory browsing (fixed, fully-enumerable category paths).
        ...
        if result is None:
            ...
            log.warning(
                "PRODUCTS_UNIFIED_NO_V2_MATCH | query=%r | subcategory=%r",
                query,
                subcategory,
            )
            result = {
