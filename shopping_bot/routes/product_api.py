...
def get_product_detail(product_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    Sections returned:
      ...
      - score_cards: named object with keys {protein, fiber, sweeteners, oils, watch_outs, calories, flean_rank}
                     (oils card title is "Fat"; watch_outs when processing_type is ultra_processed and
                     ingredients_tags has neutral/negative highlights; flean_rank only when watch_outs absent)
